...
By mirroring to definitions of \uswn{s} taking into account that now labels are on nods, a \emph{valid sequence} of \tg is a sequence $\ind{i}_0\ldots\ind{i}_n$ of nodes in $\tg.V$ leading from the initial to the accepting node traversing transitions with nonzero probability:
\begin{inparaenum}[\it (i)]
\item $\ind{i}_0 = \tg.s$;
\item $\ind{i}_n = \tg.e$;

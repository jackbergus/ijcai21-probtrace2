...
\subsection{Untimed Stochastic Petri Nets (USPN)}\label{subsec:spn}
In this paper, we consider a specific case of Untimed Stochastic Petri Nets, such as Workflow Networks over $k$-bounded Place-Transitions Nets \cite{MarsanCB84,Desel1998,RoggeSoltiAW13}. Those Untimed Stochastic Petri Nets can be modelled as a tuple $USPN=(P,T,F,\omega,W,i,f)$ where:
\begin{itemize}
...
We denote the transition from marking $M$ to marking $M'$ via an enabling $t$ as a relation $M\overset{t}{\to}M'$. We say that a USPN with initial marking $M$ is $k$-\textit{bounded} if each of the markings $M'$ reachable from $M$, $M$ included, have $\forall p\in P.\; M(p)\leq k$\\


\begin{example}
Figure \ref{fig:spn} provides an example of a Stocastic Petri Net defined as such, and \ref{fig:rg} provides its associated Reachability Graph \texttt{[TODO: some explanation?]}. This representation can be beneficial when such USPNs are inferred and extracted from log files \cite{PPNFromLog} for extracting the set of the probabilistic traces associated to the USPN.
\end{example}


In this paper, we use USPNs for modelling business processes: in fact, it can be shown (\texttt{[TODO: paper?]}) that it is always possible to convert BPMNs to USPNs. Last, in this paper we also assume that a transition is enabled when all of its input places contain at least one token and that, when a transition fires, we remove one token from each of its input places and depose tokens for each of its output places: this condition is set via $\forall e\in F. \omega(e)=1$.
...
\begin{figure}[!t]
	\centering
	\subfloat[A (Probabilistic) Transition Graph.]{\label{fig:orig}\includegraphics{images/running_example.pdf}}\qquad
	\subfloat[Another (Probabilistic) Transition Graph having the same set of weighted traces.]{\label{fig:closed}\includegraphics{images/closed_example.pdf}}\\
	\caption{Two equivalent probabilistic (Probabilistic) Transition Graphs. Initial nodes are pointed by arrows starting from $\ast$, while accepting nodes are circled.}
	\label{3figs}
\end{figure}
...
Figure \ref{fig:orig} is a  TG $P^*=(\mathtt{\color{blue}1},\mathtt{\color{blue}6},L,R,1)$, where the matrices $L$ and $R$ can be both defined as follows:
$$L:=\kbordermatrix{

...
We can also associate a weight $w\in[0,1]\subseteq\mathbb{R}$ to a PPN, so to express the probability associated with the PPN itself as valid. This representation can be beneficial when such PPNs are inferred and extracted from log files \cite{PPNFromLog}.
...
\begin{figure}[!t]
	...
	\caption{Two equivalent probabilistic Petri Nets. Initial nodes are pointed by arrows starting from $\ast$, while accepting nodes are circled.}
	\label{3figs}
\end{figure}

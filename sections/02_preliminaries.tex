...
\begin{example}
Figure \ref{fig:orig} can be represented as a PPN $(\mathtt{\color{blue}1},\mathtt{\color{blue}6},L,R,1)$, where the matrices $L$ and $R$ can be both defined as follows:
$$L:=\kbordermatrix{
...
We can easily see that the Petri Net in Figure \ref{fig:closed} produces the same set of traces, so the two Petri Nets are (probabilistic-trace) equivalent.
\end{example}

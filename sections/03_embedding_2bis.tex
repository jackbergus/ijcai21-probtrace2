...
\begin{figure*}[!t]
	...
	\subfloat[The transformed space for the $k$NN problem.]{\label{fig:knnspace}\includegraphics[width=.35\textwidth]{images/transformed_space.pdf}}\\
...
$p\cdot s\equiv \mathbb{P}_N(\sigma)\mathbb{P}_N(\sigma')\cdot s_d(\sigma,\sigma')$ (Figure \ref{fig:spp}).}
...
{Given that users might still prefer the most similar ones to the ones maximizing both probability and similarity, we %prefer to 
...
{Given that the  for each candidate trace %$\nonlogtrace$ to align to $\logtrace$  
...
\begin{example}
	{Figure \ref{fig:spp} shows a family of hyperbolae $ps$ describing all the alignments having $\mathcal{R}(\sigma,\sigma')=ps$. Point $\color{red}(1,1)$ represents the best possible trace match, i.e., a trace $\nonlogtrace\in\ptraces{N}{0}$ with $\nonlogtrace=\logtrace$ and $\mathbb{P}_N(\nonlogtrace)\mathbb{P}_N(\logtrace)=1$.

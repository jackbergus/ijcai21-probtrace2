...
Given that the embedding of a (weighted) trace $\braket{\trace,\probskip{\trace}}\in\WCal{\pmin}{n}(P)$ requires an intermediate $TG$ representation, we can map each of these traces to the subgraph of $P$ generating $\braket{\trace,\probskip{\trace}}$ with at most $|\trace|$ steps:}
	\begin{definition}[TG projection over traces]
		Given a minimum probability threshold $\pmin$, a maximum path length $n$, and a TG $P=(s,t,L,R,\omega)$, we generate a set $\TBf{\pmin}{n}(P)$ of projected TGs $P$ for each trace in $\WCal{\pmin}{n}(P)$ as follows: for each weighted trace $\braket{\trace,\probskip{\trace}}\in\WCal{\pmin}{n}(P)$ generated from a path $\pi_\trace=s\to n_2\rightsquigarrow n_m\to t$ over $R$, we generate a TG $P_\trace=(s',t',L_\trace,R_\trace,\omega')$, where \begin{alphalist}

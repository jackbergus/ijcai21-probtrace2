...
\ifdefined\EqualityHolds\newcommand{\probarg}{\net}\else \newcommand{\probarg}{\closed{\tg_{\rg{\net}}}}\fi
...
%	
If we want to align the trace $\logtrace$ over the \unravelled\ traces $\mathcal{X}=\WCal{p_\theta}{n}(P)$ and $d$ is a distance, the $k$-nearest neighbors describe the best $k$ alignments for $\logtrace$.} We now discuss two different possible strategies for obtaining the top-$k$ model trace alignments.
...
If we want to represent the same intuition of such a weighted distance as a kernel function to induce a ranking, we need to first transform it as a similarity function returning $1$ when $\trace=\logtrace$ and has probability $\probskip{\trace}=1$. We then express $d$ as}
...
Given that the embedding of a (weighted) trace $\braket{\trace,\probskip{\trace}}\in\WCal{p_\theta}{n}(P)$ requires an intermediate $TG$ representation, we can map each of these traces to the subgraph of $P$ generating $\braket{\trace,\probskip{\trace}}$ with at most $|\trace|$ steps:}
	\begin{definition}[TG projection over traces]
		Given a minimum probability threshold $p_\theta$, a maximum path length $n$, and a TG $P=(s,t,L,R,\omega)$, we generate a set $\TBf{p_\theta}{n}(P)$ of projected TGs $P$ for each trace in $\WCal{p_\theta}{n}(P)$ as follows: for each weighted trace $\braket{\trace,\probskip{\trace}}\in\WCal{p_\theta}{n}(P)$ generated from a path $\pi_\trace=s\to n_2\rightsquigarrow n_m\to t$ over $R$, we generate a TG $P_\trace=(s',t',L_\trace,R_\trace,\omega')$, where \begin{alphalist}
...
\begin{table}[!t]
	\caption{Generating the sub-TGs $P_\trace\in \TBf{\varepsilon}{4}(P)$ from the TG $P$ via the set of weighted traces $\WCal{\varepsilon}{4}(P)$, where $\varepsilon$ is the machine epsilon. $l$ and $w$ respectively represent the desired parameter for $\phi_{\mathcal{P}}(P_\trace)$ and the weight associated to $P_\trace$.}\label{tab:proj}
	\centering
	\resizebox{.45\textwidth}{!}{\begin{tabular}{>{\centering\arraybackslash} m{1cm}| >{\centering\arraybackslash} m{4cm} >{\centering\arraybackslash} m{1cm} >{\centering\arraybackslash} m{1cm} }
			...
			\midrule
			a & \includegraphics{images/trace_a} & $1$ & $\color{violet}\pa\pf$\\
			cb & \includegraphics{images/trace_cb} & $2$ & $\color{violet}\pb$\\
			aaa & \includegraphics{images/trace_a_loop} & $3$ & $\color{violet}\pa\pf$\\
			caa & \includegraphics{images/trace_caa} & $3$ & $\color{violet}\pb\pf$\\
			\bottomrule
	...
	$\trace$&$P_\trace$&$l$&$\omega$\\
	\midrule
	aa & \includegraphics{images/trace_aa} & $2$ & $\color{violet}\pa\pf$\\
	ca & \includegraphics{images/trace_ca} & $2$ & $\color{violet}\pb\pf$\\
	\begin{tabular}{l}aaaa\end{tabular} & \includegraphics{images/trace_a_loop} & $4$ & $\color{violet}\pa\pf$\\
	caaa & \includegraphics{images/trace_ca_loop} & $4$ & $\color{violet}\pb\pf$\\
	\bottomrule
\end{tabular}}
\end{table}
...
%	$\phi_{\mathcal{P}}(P_{\logtrace})$ & $8.16\cdot10^{-17}$ & $4.08\cdot 10^{-17}$ & $4.08\cdot10^{-17}$ & $4.37\cdot 10^{-17}$ & $1.03\cdot10^{-16}$ & $4.37\cdot10^{-17}$\\
%	\bottomrule
%\end{tabular}
%\end{table}
...
\begin{table}[!t]
	...
	\begin{tabular}{l|l|l|l|l|l|l|}
		...
		$\phi_{\mathcal{P}}(P_\const{aaaa})$ & $1.00\cdot 10^{-24}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $6.44\cdot 10^{-26}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\phi_{\mathcal{P}}(P_\const{aaa})$ & $1.00\cdot 10^{-24}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $1.29\cdot 10^{-25}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\phi_{\mathcal{P}}(P_\const{aa})$ & $1.00\cdot 10^{-24}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $2.57\cdot 10^{-25}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\phi_{\mathcal{P}}(P_\const{a})$ & $1.00\cdot 10^{-4}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\phi_{\mathcal{P}}(P_\const{caa})$ & $7.07\cdot 10^{-25}$ & $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$& $1.46\cdot 10^{-25}$& $2.05\cdot 10^{-25}$& $0.00\cdot 10^{0}$\\
		$\phi_{\mathcal{P}}(P_\const{ca})$ & $7.07\cdot 10^{-25}$ & $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$& $0.00\cdot 10^{0}$& $1.00\cdot 10^{-8}$& $0.00\cdot 10^{0}$\\
		$\phi_{\mathcal{P}}(P_\const{cb})$ &  $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$ & $7.07\cdot 10^{-25}$& $0.00\cdot 10^{0}$&  $0.00\cdot 10^{0}$ & $4.29\cdot 10^{-9}$\\
		$\phi_{\mathcal{P}}(P_\const{caaa})$  & $7.07\cdot 10^{-25}$ &  $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$& $1.03\cdot 10^{-25}$&  $7.20\cdot 10^{-26}$ & $0.00\cdot 10^{0}$\\
		\bottomrule
				$\phi_{\mathcal{P}}(P_{\logtrace})$ & $8.16\cdot10^{-17}$ & $4.08\cdot 10^{-17}$ & $4.08\cdot10^{-17}$ & $4.37\cdot 10^{-17}$ & $1.03\cdot10^{-16}$ & $4.37\cdot10^{-17}$\\
		\bottomrule
	\end{tabular}
...
\begin{table}[!t]
	...
	%		\toprule
	%		$\trace$ & $k_{\phi_{\mathcal{P}}}(\trace,\logtrace)$ & \textit{kernel ranking} & expected ranking\\
	%		\midrule
	...
	\resizebox{.9\textwidth}{!}{\begin{tabular}{l|ll|cc}
		...
		{$k_{\phi_{\mathcal{P}}}(P_\trace,P_{\logtrace})$} \\
...
\end{tabular}\quad	\begin{tabular}{l|c}
	...
	{$\trace$} &
	{$\boldsymbol{\downarrow}k_{\phi_{\mathcal{P}}}(P_\trace,P_{\logtrace})$} \\
...
%		\multirow{2}{*}{$k_{\phi_{\mathcal{P}}}(\trace,\logtrace)$}&
...
%		%\hline
%		$\trace$ &  $k_{\phi_{\mathcal{P}}}(P_\trace,P_{\logtrace})$ & $k_\star(\trace,\logtrace)$\\ %&
%		%$\trace$ &  $k_{\phi_{\mathcal{P}}}(\trace,\logtrace)$ & \textit{exp. ranking}\\
%		\midrule
%		

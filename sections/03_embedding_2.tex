...
\newcommand{\logtrace}{\trace_\ell}
\subsection{Alignment Strategy}\label{subsec:as}
Despite current trace alignment algorithms return a single alignment for a single model trace, approaches generating different alignment possibilities would provide the user with a richer feedback. This is even more important for approaches based o stochastic nets, like \uswn-s, since, in this case, distinct model traces have different probabilities and the retrieval of the best model trace maximizing the combined provision of minimum trace alignment cost and maximum model trace probability might not suffice. Therefore, we would
like to return the best $k$ alignments among all the distinct \unravelled\ model traces in $\ptraces{\net}{\pmin}=\ptraces{\closed{\tg_{\rg{\net}}}}{\pmin}$. In the database community, this problem is usually tackled via $k$-Nearest Neighbors.
...
%	
If we want to align the trace $\logtrace$ over the \unravelled\ traces $\mathcal{X}=\WCal{\pmin}{n}$ and $d$ is a distance, the $k$-nearest neighbors describe the best $k$ alignments for $\logtrace$.} We now discuss two different possible strategies for obtaining the top-$k$ model trace alignments.

\subsubsection{Optimal-Ranking Trace Aligner}\label{subsec:eta}
{A possible way to carry out probabilistic trace alignment is to reuse existing trace aligners such as \cite{LeoniM17}, where the notion of distance $d(\trace,\logtrace)$ between two traces $\trace$ and $\logtrace$ is the Levenshtein distance, i.e., the {minimum} cost for aligning one of the two traces with respect to the other when all the possible moves have cost equal to 1.\footnote{This distance is slightly different from the Levenshtein distance known in the literature since here the replacement is obtained as a concatenation of a deletion and an addition.} We can then exploit decision theory \cite{dectheor} and express the decision score as  $\probskip{\trace} d(\trace,\logtrace)$.

If we want to represent the same intuition of such a weighted distance as a kernel function to induce a ranking, we need to first transform it as a similarity function returning $1$ when both $\trace=\logtrace$ and  $\probskip{\trace}=1$ hold. We express $d$ as}
...
a normalized similarity score $s_d(\trace,\trace'):=1/(d(\trace,\trace')/c+1)$ where $c\in\mathbb{N}_{\neq 0}$ is a constant, so that the maximum similarity of $1$ is reached when the distance is $0$ and the similarity decreases while the distance increases \cite{BergamiBM20}. %\xout{As a consequence, the exact trace aligner will find the weighted trace $\braket{\trace,\mathbb{P}(\trace)}$ in $\mathcal{W}_{\pmin}(P)$ maximising $s_d(\trace,\logtrace)$, and use $s_d$ as a ranking function.} \ADD
{By  $\probskip{\logtrace}=1$, we can express the golden kernel function (i.e., the one producing the optimal ranking) as $k_\star(\trace,\logtrace)=\probskip{\trace} \probskip{\logtrace} s_d(\trace,\logtrace)$. The computation $\underset{\braket{\trace,\probskip{\trace}}\in \WCal{\pmin}{n}(P)}{\max\arg} k_\star(\trace, {\logtrace})$ returns the best optimal-ranking trace alignment $\trace$ for a query trace $\logtrace$, where $k_\ast$ must be computed a-new for all the possible $\logtrace$.
	
	
	

\subsubsection{Approximate-Ranking Trace Embedder}\label{subsec:ate}
As previously observed, $k$NN ranking optimality comes at the sub-optimal cost of a brute-force recomputation of all the embeddings $\phi$ for each novel trace $\logtrace$ to align\footnote{By embedding  all the \unravelled\ model traces via $\phi_{\logtrace}(\braket{\trace,\probskip{\trace}})=\Big(\frac{1}{\probskip{\trace}\sqrt{\probskip{\trace}^2+s_d^2(\trace,\logtrace)}},\frac{1}{s_d(\trace,\logtrace)\sqrt{\probskip{\trace}^2+s_d^2(\trace,\logtrace)}}\Big)$ over a specific $\logtrace$, by representing all the $\logtrace$ as $(0,0)$, and by using the Euclidean Distance as a distance, we ensure that all the neighbors of $(0,0)$ will represent the top-$k$ best alignments for $\logtrace$.}. %This is not optimal for $k$NN-based techniques, as this entails the  redefinition of the multidimensional search space $\mathcal{X}$ for each new alignment $\logtrace$. The ``recomputation cost'' could be completely avoided by providing an embedding strategy $\gorgembed$ that is independent from the to-be-aligned trace $\logtrace$.
An embedding strategy 
Given that each embedding $\phi$ entails an associated similarity metric $k_\phi$ (\S\ref{subsec:katk}) and therefore an associated distance $d_{k_\phi}$ (Equation \ref{eq:dofk}), we can compute all the embeddings $\gorgembed$ for all the \unravelled\ traces before performing the top-$k$ search, thus avoiding the brute-force cost. This computational gain comes with a loss in precision, as the generation of precise embeddings for graph data with loops is an NP-Complete (thus intractable) problem \cite{GartnerFW03} and, in its approximated version, is not able to accurately represent the data using low-dimensional vectors \cite{Seshadhri5631}. So, our proposed definition of $\gorgembed$ performs  weakly-ideally (\S\ref{subsec:katk} and \cite{Gartner03}).

{The embedding strategy} $\trembed$ (\S\ref{subsec:katk} and \cite{LodhiSSCW02,Raedt}9 {from current literature has several short\-comings: firstly,} \begin{alphalist} \item it does not perform weakly-ideally, so we cannot numerically assess when two embedding represent equivalent traces (Example \ref{ex:wheredotiszero}). Secondly, \item it does not characterize $\tau$-moves, so initial $\tau$-moves' probabilities are not preserved (Example \ref{ex:neue}). Thirdly, \item it is affected by numerical errors from finite arithmetics (e.g., double, float): longer traces $\trace$ generated from skewed probability distributions $\expN.\Lambda^i$ will suffer from greater truncation errors, as smaller $\lambda^i$ components for bigger $i<|\trace|$ will be ignored, thus preventing a complete numerical vector characterization of  $\trace$ in practice.
\end{alphalist}

{We overcome these shortcomings by} \begin{alphalist} \item proposing a weakly-ideal embedding (p. \pageref{subsub:prop}), which also \item exploits an $\omega$ factor for preserving probabilities from and to $\tau$ transitions (Example \ref{ex:neue}). Furthermore, \item we mitigate the numerical truncation errors induced by trace length and probability distribution's skewness by introducing two sub-embedding strategies, $\epsilon^x$ and $\nu^x$, where the former descends from $\trembed$ and the latter approximates the trace similarity via label frequencies' similarity.
\end{alphalist}

Given that a trace embedding $\trace\in\WCal{\pmin}{n}$ adequately representing the transitions in $\expN.\Lambda$ requires an intermediate $G$ representation, we can map each trace to a transition graph $\tg_\sigma$ as follows:}
	\begin{definition}[$\closed{G}$ projection over traces]
		Given a minimum probability threshold $\pmin$ and a $\tau$-closed transition graph $\closed{G}$, for each trace $\trace\in\ptraces{\closed{G}}{\pmin}$ we generate a transition graph
		$\closed{G}_\sigma=(V_\trace,s_\trace,t_\trace,L_\trace,R_\trace,\omega_\trace)$, where \begin{inparaenum}[\it (i)]
		\item $V_\trace$ contains all the distinct transitions 	
		\end{inparenum}
		
		we generate a set $\TBf{\pmin}{n}(\closed{G})$ of projected TGs $P$ for each trace in $\WCal{\pmin}{n}(P)$ as follows: for each weighted trace $\braket{\trace,\probskip{\trace}}\in\WCal{\pmin}{n}(P)$ generated from a path $\pi_\trace=s\to n_2\rightsquigarrow n_m\to t$ over $R$, we generate a TG $P_\trace=(s',t',L_\trace,R_\trace,\omega')$, where \begin{inparaenum}[\it (i)]
			\item $s'=s$ if $\textit{label}(s)\neq \tau$ and $t'=n_2$ otherwise,
			\item $t'=t$ if $\textit{label}(t)\neq \tau$ and $t'=n_m$ otherwise,
			\item $L_\trace$ (and $R_\trace$) is the submatrix of $L$ (and $R$) over the non-$\tau$ labeled notes in $\pi_\trace$ and the labels from $\trace$,
...
\begin{table}[!t]
	\caption{Generating the sub-TGs $P_\trace\in \TBf{\varepsilon}{4}(P)$ from the TG $P$ via the set of weighted traces $\WCal{\varepsilon}{4}(P)$, where $\varepsilon$ is the machine epsilon. $l$ and $w$ respectively represent the desired parameter for $\gorgembed(P_\trace)$ and the weight associated to $P_\trace$.}\label{tab:proj}
	\centering
...
{We also require that the desired properties of $\gorgembed$ are independent from the characterization of $\epsilon$ and $\nu$, which  provide different embedding strategies. } The final embedding is defined as follows:
...
Given a $\tau$-closed transition graph $\closed{\tg}$ and a tuning parameter $t_f\in[0,1]\subseteq\mathbb{R}$, the associated embedding is defined as follows:
$$\gorgembed_{i}(\closed{\tg})=\begin{cases}
	\omega t_f^{|R>0|}\,\sfrac{\epsilon_\const{ab}(\closed{\tg})}{\|\epsilon\|_2} & {i}=\const{ab}\\
	\;\;\,t_f^{|R>0|}\,\sfrac{\nu_\const{a}(\closed{\tg})}{\|\nu\|_2} & {i}=\const{a}\\
\end{cases}$$
where $\epsilon$ (and $\nu$) represents the non-negatively defined embedding associated to $\closed{\tg}.R$ (both $\closed{\tg}.R$ and $\closed{\tg}.L$).
\end{definition}

At this stage, the computation of $\underset{\trace\in \WCal{p}{n}, P_\trace\in\TBf{p}{n}(P)}{\max\arg} k_{\gorgembed}(P_\trace, P_{\logtrace})$ returns the best approximated trace alignment $\trace$ for a query trace represented as $P_{\logtrace}$. %\xout{Similarly, we can provide the TG $P\in\mathbf{P}$ providing the best approximated alignment for $P_{\logtrace}$ as $\underset{P}{\max\arg}\underset{ P_\trace\in\mathbf{P}_p^n(P)}{\max} k_{\gorgembed}(P_\trace, P_{\logtrace})$.}¯
...
\begin{table}[!t]
	\centering
	\caption{Embedding for both the traces $\trace$ of maximum length $4$ from $P$ and  $\logtrace$.}\label{tab:emb2}\label{tab:embsitar}
	\begin{tabular}{l|l|l|l|l|l|l|}
		\toprule
		& $\const{a}$    & $\const{b}$                                                   & $\const{c}$    & $\const{aa}$   & $\const{ca}$   & $\const{cb}$   \\
		\midrule
		
		
		$\gorgembed(P_\const{aaaa})$ & $1.00\cdot 10^{-24}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $6.44\cdot 10^{-26}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\gorgembed(P_\const{aaa})$ & $1.00\cdot 10^{-24}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $1.29\cdot 10^{-25}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\gorgembed(P_\const{aa})$ & $1.00\cdot 10^{-24}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $2.57\cdot 10^{-25}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\gorgembed(P_\const{a})$ & $1.00\cdot 10^{-4}$ & $0.00\cdot 10^{0}$ & $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$& $0.00\cdot 10^{0}$\\
		$\gorgembed(P_\const{caa})$ & $7.07\cdot 10^{-25}$ & $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$& $1.46\cdot 10^{-25}$& $2.05\cdot 10^{-25}$& $0.00\cdot 10^{0}$\\
		$\gorgembed(P_\const{ca})$ & $7.07\cdot 10^{-25}$ & $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$& $0.00\cdot 10^{0}$& $1.00\cdot 10^{-8}$& $0.00\cdot 10^{0}$\\
		$\gorgembed(P_\const{cb})$ &  $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$ & $7.07\cdot 10^{-25}$& $0.00\cdot 10^{0}$&  $0.00\cdot 10^{0}$ & $4.29\cdot 10^{-9}$\\
		$\gorgembed(P_\const{caaa})$  & $7.07\cdot 10^{-25}$ &  $0.00\cdot 10^{0}$ & $7.07\cdot 10^{-25}$& $1.03\cdot 10^{-25}$&  $7.20\cdot 10^{-26}$ & $0.00\cdot 10^{0}$\\
		\bottomrule
				$\gorgembed(P_{\logtrace})$ & $8.16\cdot10^{-17}$ & $4.08\cdot 10^{-17}$ & $4.08\cdot10^{-17}$ & $4.37\cdot 10^{-17}$ & $1.03\cdot10^{-16}$ & $4.37\cdot10^{-17}$\\
		\bottomrule
	\end{tabular}
...
Given $\trace_\tau=\Set{a,b,c}$, the embedding space is of size $6$: three features (computed using $\nu$) correspond to the labels of the non-$\tau$ vertices in $\trace_\tau$, i.e., $\{a,b,c\}$, and other three features (computed using $\epsilon$) correspond to the $2$-grams subsequences that are also traces in $W^4_0(P)$, i.e., $\{aa,ca,cb\}$. Therefore, $\{a,b,c,aa,ca,cb\}\subset \trace_\tau^2\cup\trace_\tau$ is the whole set of features describing both the label and the transition matrix, so $\gorgembed$ is a vector with 6 dimensions.
...
Table \ref{tab:emb2} represents the embeddings $\gorgembed(P_\trace)$ generated from the sub-TGs generated in Example \ref{ex:neue}, where the $l=|\trace|$ for each trace $\trace$ associated to the sub-TG. This representation is completely independent from the representation associated with a trace to be aligned. Therefore it does not have to be recomputed at each alignment with a different $\logtrace$.
{After representing trace $\logtrace=\textup{caba}$ to be aligned as a graph (see Example \ref{ex:tracembed}), we can represent its}
...
\begin{table}[!t]
	\caption{Comparison between the ranking induced by the expected ranking $k_\star$ and the proposed kernel $k_{\gorgembed}$ with embedding strategies $\epsilon^1$ and $\nu^1$: arrows $\boldsymbol{\downarrow}$ remark the column of choice under which we sort the rows (i.e., ranking).}\label{tab:rank3}
	\centering
	...
	{$\trace$} &
	{$\boldsymbol{\downarrow}k_\star(\trace,\logtrace)$} \\
...
\begin{example}\label{ex:11}
	Given that $k_{\gorgembed}(\trace,\logtrace)=\braket{\gorgembed(P_\trace),\;\gorgembed(P_{\logtrace})}$ for each weighted trace $\braket{\trace,\probskip{\trace}}\in\WCal{\varepsilon}{4}(P)$, then the dot product between the resulting similarity ranking is represented in Table \ref{tab:rank3}, where the expected ranking $k_\star$ is also showed. This similarity score approximates the expected ranking  and tends to rank in similar ways the paths generated from the same subgraph of the TG.
\end{example}
...
%\end{table}
\begin{table}[!t]
	\caption{Comparison between the ranking induced by the expected ranking $k_\star$ and the proposed kernel $k_{\gorgembed}$ with embedding strategies $\epsilon^2$ and $\nu^1$: arrows $\boldsymbol{\downarrow}$ remark the column of choice under which we sort the rows (i.e., ranking).}\label{tab:compLit}
	\centering
	\resizebox{.9\textwidth}{!}{\begin{tabular}{l|ll|cc}
			\toprule
...
%		%\hline
%		$\trace$ &  $k_{\gorgembed}(P_\trace,P_{\logtrace})$ & $k_\star(\trace,\logtrace)$\\ %&

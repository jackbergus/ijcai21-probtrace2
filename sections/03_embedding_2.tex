...
\begin{figure*}[!t]
	...
	\subfloat[The transformed space for the $k$ nearest neighbours problem.]{\label{fig:knnspace}\includegraphics[width=.45\textwidth]{images/transformed_space.pdf}}\\
...
\ADD{Table \ref{tab:expected} represents the expected alignment raking of such traces with  $\nonlogtrace=\textup{caba}$.  Albeit traces $\const{caa}$ and $\const{caaa}$ are the most similar to $\const{caba}$, their associated trace probability  is rather low, so traces having higher probability but lower similarity score are preferred (e.g., $\const{a}$ and $\const{aa}$).}
\end{example}
	
\ADD{Given that users might still prefer the most similar ones to the ones maximizing both probability and similarity, we prefer to offer the user the whole set of the best $k$ solutions rather than cherry-picking the best solution maximizing both probability and similarity. Therefore, we can reduce the problem to the desired $k$-nearest neighbor problem within the Euclidean Space: we define a transformation $t$ such that the distance of the transformed point $t(p,s)$ from  the origin $\vec{0}$ corresponds to $\sfrac{1}{ps}$, thus preserving the score from the golden ranking and making the points maximizing $ps$ nearer to the origin of the axes. We then chose the following transformation:}
\[t(p,s):=\left(\frac{1}{s\sqrt{p^2+s^2}},\; \frac{1}{p\sqrt{p^2+s^2}}\right)\]
	\ADD{Given that the  for each candidate trace $\nonlogtrace$ to align to $\logtrace$  we always start looking for the candidates from the origin of the axes, our search will always start scanning from the best candidate first.}

	
\begin{example}
	\ADD{Figure \ref{fig:spp} shows a family of hyperbolae $\mathbb{P}_N(\sigma)\mathbb{P}_N(\sigma')\cdot s(\sigma,\sigma')=ps$ describing all the points $(\mathbb{P}_N(\sigma)\mathbb{P}_N(\sigma'), s(\sigma,\sigma'))$ having $ps$ as a $\mathcal{R}$ score. Point $\color{red}(1,1)$ represents the best possible trace match, as it means that there exists a trace $\nonlogtrace\in\ptraces{N}{0}$ with $\nonlogtrace=\logtrace$ and $\mathbb{P}_N(\nonlogtrace)\mathbb{P}_N(\logtrace)=1$.
		
		
	Figure \ref{fig:knnspace} shows that the aforementioned embedding moves the points of the hyperbola $\mathbb{P}_N(\sigma)\mathbb{P}_N(\sigma')\cdot s(\sigma,\sigma')=ps$ over a circumference $x^2+y^2=\sfrac{1}{(ps)^2}$ describing a locus of the points equidistant from the origin of the axes $(0,0)$ with distance $\sfrac{1}{ps}$.}
\end{example}

The following lemma guarantees that the transformation required for running the $k$NN algorithm preserves the ranking induced by $\mathcal{R}$.
...
\begin{proof}
	...
	\ADD{We can observe that both $\epsilon^1$ and $\epsilon^2$ are function of both the sub-traces of length $i$ from $1$ to $l$, and  $[LR^iL^t]_\const{ab}$ too:}
	\begin{itemize}
...
\ADD{As a corollary of Proposition~\ref{lem:rewritinglemma}, we can also express the kernel $k_{\gorgembed}(G,G')$ is a function of the dot product of the two sub-embeddings because $\tasks^2\neq\tasks$ is always true:}
...
\end{lemma}
\begin{proof}
	\ADD{For each transition graph $G$, we can denote the set of vertex labels as $\tasks_G=\Set{\const{a}\in\tasks|\forall u\in V.\; L_{\const{a}u}\neq 0}$.} If we exclude the trivial conditions $t_f=0$, $\omega=0$ or $\omega'=0$ and by Equation \ref{eq:corollLem1}, the only condition when the kernel returns zero is when both $\Braket{\hat{\epsilon}(G),\hat{\epsilon}(G)}=0$ and $\Braket{\hat{\nu}(G),\hat{\nu}(G')}=0$ hold. \ADD{This reduces to prove that  iff. $G$ and $G'$ have either different $2$-gram sets or a different set of vertex labels, i.e., $\tasks_G\cap \tasks_{G'}=\emptyset$.}

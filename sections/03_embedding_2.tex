...
\begin{inparaenum}[\it (i)]
	...
	\item $\closed{G}_\nonlogtrace.L$ (and $\closed{G}_\nonlogtrace.R$) is the submatrix of $L$ (and $R$) over the columns (and rows) in $\closed{G}_\sigma.V$, and $\closed{G}_\sigma.\omega$ preserves the transition probabilities from and to $\tau$ initial nodes:
	 $1-\prod_{\xi\in\runs{\sigma'}{\closed{G}},\eta\in\seqs{\xi}{\closed{G}}}\Big(1-(\textit{ifte}([L]_{\tau\eta_1},[R]_{\eta_1\eta_2})$
	 $\cdot\textit{ifte}([L]_{\tau t_n},[R]_{t_{n-1} t_n})\Big)$,
...
for embedding them is displayed (e.g., all the $\tau$-labeled nodes are removed). The associated weight $\omega$ derives
...
{We now focus on the embedding $\gorgembed$ for each transition graph generated in the former definition. The goal is to use
$k_{\gorgembed}$ for ranking all the traces generated by \unravelling via such graphs. We extend  previous work on embeddings by including
...
{We also require that the desired properties of $\gorgembed$ are independent from the characterization of $\epsilon$ over the $2$-grams in $\mathcal{A}^2$ and $\nu$ over the labels in $\mathcal{A}$, which  provide different embedding strategies. } The final embedding is defined as follows.
...
Table \ref{tab:emb2} shows the embeddings $\gorgembed(G_\nonlogtrace)$ generated from 
...
{as} in Table \ref{tab:embsitar}: $\const{a}$ is clearly the most frequent label to $\const{b}$ and $\const{c}$ that are equiprobable, and $2$-gram $\const{ca}$ appears twice in the trace set and so, it is more frequent than the other $2$-grams.
\end{example}
%
...
Given  $\overline{G}_{\sigma'}=(s,t,L,R,w)$ and $\overline{G}_{\sigma''}=(s',t',L',R',w')$, the TG Kernel is defined as follows:
$
k_{\gorgembed}(\overline{G}_{\sigma'},\overline{G}_{\sigma''})=\omega'\omega''t_f^{|R'>0|+|R''>0|}\left(1-\frac{\norm{\hat{\epsilon}'-\hat{\epsilon}''}{2}^2}{2}\right)+t_f^{|R'>0|+|R''>0|}\left(1-\frac{\norm{\hat{\nu}'-\hat{\nu}''}{2}^2}{2}\right)\\$
\end{proposition}

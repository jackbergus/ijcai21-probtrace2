...
The embedding $\phi_{\mathcal{T}}$ \cite{LodhiSSCW02,Raedt} has several shortcomings: one of them is that \begin{alphalist} \item such embedding does not perform weakly-ideally, thus implying that we cannot numerically assess when two embedding represent equivalent traces (Example \ref{ex:wheredotiszero}). Secondly, \item such embedding does not  characterize $\tau$-moves, and therefore we need to use an $\omega$ value to preserve the probability of initial $\tau$-moves that do not produce visible transitions (Example \ref{ex:neue}). Third, \item current literature does not take into consideration numerical errors coming from the finite arithmetics representation of numerical vecotrs (e.g., double, float): longer traces $\sigma$ will suffer from truncation errors, as smaller $\lambda^i$ components for bigger $i<|\sigma|$ will be ignored, thus preventing a complete numerical characterization of  $\sigma$.
\end{alphalist}

We overcome these shortcomings by \begin{alphalist}

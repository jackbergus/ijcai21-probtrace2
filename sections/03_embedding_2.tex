...
\begin{table}[!t]
	...
	\resizebox{.6\textwidth}{!}{\begin{tabular}{c|c|c}
		...
		$\nu^x(P)_{\color{green}\alpha}:=$ & $\frac{1}{N}\sum_{\braket{\trace,w}\in\WCal{\pmin}{l}(P)}\frac{|\Set{\trace_i\in\trace|\trace_i\neq\tau\wedge \trace_i={\color{green}\alpha}}|}{|\trace|}$ & $0$ \\
		\bottomrule
	\end{tabular}}
\end{table}
...
\begin{example}\label{ex:11}
	Given $k_{\gorgembed}(\trace,\logtrace)=\braket{\gorgembed(G_\trace),\;\gorgembed(G_{\logtrace})}$ with sub-embedding $\nu^1$ and $\epsilon^1$ for each  trace $\trace\in\WCal{\varepsilon}{4}$,  the dot product between the resulting similarity ranking is represented in Table \ref{tab:rank3}, where the expected ranking $k_\star$ is also showed. $k_{\gorgembed}$ approximates the expected ranking  and tends to rank in similar ways the paths generated from the same subgraph of the TG.
\end{example}

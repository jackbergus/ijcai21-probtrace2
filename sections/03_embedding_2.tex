...
\begin{figure*}[!t]
	\centering
	\subfloat[The similarity/probability space.]{\label{fig:spp}\includegraphics[width=.35\textwidth]{images/original_space.pdf}}\qquad
	\subfloat[The transformed space for the $k$ nearest neighbours problem.]{\label{fig:knnspace}\includegraphics[width=.35\textwidth]{images/transformed_space.pdf}}\\
...
\ADD{Given that users might still prefer the most similar ones to the ones maximizing both probability and similarity, we %prefer to 
return the %user the whole set of the 
best $k$ solutions. %rather than cherry-picking the best solution maximizing both probability and similarity. 
So, we reduce the problem to the desired $k$-nearest neighbor problem within the Euclidean Space: we define a transformation $t$ such that the distance of the transformed point $t(p,s)$ from  the origin $\vec{0}$ corresponds to $\sfrac{1}{ps}$, thus preserving the score from the golden ranking and making the points maximizing $ps$ nearer to the origin of the axes. We then chose the following transformation:}
$t(p,s):=\left(\frac{1}{s\sqrt{p^2+s^2}},\; \frac{1}{p\sqrt{p^2+s^2}}\right)$. 
	\ADD{Given that the  for each candidate trace %$\nonlogtrace$ to align to $\logtrace$  
	we always start %looking for the candidates 
	from the origin of the axes, our search always starts scanning from the best candidates.}

	
\begin{example}
	\ADD{Figure \ref{fig:spp} shows a family of hyperbolae $ps$ describing all the alignments having $\mathcal{R}(\sigma,\sigma')=ps$. Point $\color{red}(1,1)$ represents the best possible trace match, i.e., a trace $\nonlogtrace\in\ptraces{N}{0}$ with $\nonlogtrace=\logtrace$ and $\mathbb{P}_N(\nonlogtrace)\mathbb{P}_N(\logtrace)=1$.
		
		
	Figure \ref{fig:knnspace} shows that the aforementioned embedding moves the points of the hyperbola $ps$ over a circumference $x^2+y^2=\sfrac{1}{(ps)^2}$ describing a locus of the points equidistant as $\sfrac{1}{ps}$ from the origin of the axes $(0,0)$.}
\end{example}

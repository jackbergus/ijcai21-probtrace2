...
{A possible way to carry out probabilistic trace alignment is to reuse existing trace aligners such as \cite{LeoniM17}, where the notion of distance $d(\nonlogtrace,\logtrace)$ between two traces $\nonlogtrace$ and $\logtrace$ is the Levenshtein distance, i.e., the {minimum} cost for aligning one of the two traces with respect to the other when all the possible moves have cost equal to 1.\footnote{This distance is slightly different from the Levenshtein distance known in the literature since here the replacement is obtained as a concatenation of a deletion and an addition.} We can then exploit decision theory \cite{dectheor} and express the decision score as  $\probskip{\nonlogtrace} d(\nonlogtrace,\logtrace)$.

If we want to represent the same intuition of such a weighted distance as a kernel function to induce a ranking, we need to first transform it as a similarity function returning $1$ when both $\nonlogtrace=\logtrace$ and  $\probskip{\logtrace}=1$ hold. We express $d$ as}
...
Given that a trace embedding $\nonlogtrace\in\WCal{\pmin}{n}$ adequately representing the transitions in $\expN.\Lambda$ requires an intermediate $G$ representation, we can map each trace to a transition graph $\tg_\sigma$ as follows:}
	\begin{definition}[$\closed{G}$ projection over traces]
		Given a function 
		
		
		
		Given a minimum probability threshold $\pmin$ and a $\tau$-closed transition graph $\closed{G}=(V,s,t,L,R,\omega)$, for each trace $\nonlogtrace\in\ptraces{\closed{G}}{\pmin}$ we generate a transition graph
$\closed{G}_\nonlogtrace$, where \begin{inparaenum}[\it (i)]
	\item $\closed{G}_\sigma.V$ contains all the distinct nodes generating $\nonlogtrace$ from $\closed{G}$	(namely, $\bigcup_{\xi\in\runs{\closed{G}}{\nonlogtrace}}\seqs{ \closed{G}}{\xi}$) \item $\closed{G}_\sigma.s = s$ \item  $\closed{G}_\sigma.t = t$ \item $\closed{G}.L$ (and $\closed{G}.R$) is the submatrix of $L$ (and $R$) over the columns (or columns and rows) in $\closed{G}_\sigma.V$, and $\closed{G}_\sigma.\omega$ preserves the transition probabilities from and to $\tau$ initial nodes as follows: $1-\prod_{\xi\in\runs{\closed{G}}{\sigma'}\textup{ and }t_1,\dots,t_n\in\seqs{\closed{G}}{\xi}}(1-(\textit{ifte}([L]_{\tau\eta_1},[R]_{\eta_1\eta_2})\textit{ifte}([L]_{\tau t_n},[R]_{t_{n-1} t_n}))$
	where $\textit{ifte}(x,y)=x(y-1)+1$ returns $y$ when $x=1$ and, for $x=0$, returns $1$.
\end{inparaenum}
%\dots
%		
...
\begin{table}[!t]
	\caption{Generating the sub-TGs $P_\nonlogtrace\in \TBf{\varepsilon}{4}(P)$ from the TG $P$ via the set of weighted traces $\WCal{\varepsilon}{4}(P)$, where $\varepsilon$ is the machine epsilon. $l$ and $w$ respectively represent the desired parameter for $\gorgembed(P_\nonlogtrace)$ and the weight associated to $P_\nonlogtrace$.}\label{tab:proj}
	\centering
	\resizebox{.4\textwidth}{!}{\begin{tabular}{>{\centering\arraybackslash} m{1cm}| >{\centering\arraybackslash} m{4cm} >{\centering\arraybackslash} m{1cm} >{\centering\arraybackslash} m{1cm} }
			...
			\bottomrule
	\end{tabular}}\quad 	\resizebox{.4\textwidth}{!}{\begin{tabular}{>{\centering\arraybackslash} m{1cm}| >{\centering\arraybackslash} m{4cm} >{\centering\arraybackslash} m{1cm} >{\centering\arraybackslash} m{1cm} }
	\toprule
	$\nonlogtrace$&$P_\nonlogtrace$&$l$&$\omega$\\
	\midrule
...
\end{tabular}}
\end{table}
\begin{example} \small\label{ex:neue}
...
%\xout{selection of a trace $\nonlogtrace$ implies performing a set of specific probabilistic choices, we can remove all the edges starting (and arriving) at nodes that are $\tau$-labeled and move their score as the weight of the TG.}
\end{example}
...
%\xout{Given that we previously observed that a TGs $\closed{\tg_{\rg{\net}}}$ can be fully characterised (read, similarity) by their associated set of traces $\mathcal{W}_p^n(P)$  and that the trace embedding can be described as an embedding over a TG, we can characterise a TG embedding as a transition matrix embedding. In addition to that, when two Workflow Nets share similar node labelings but no ${\color{green}\alpha}\rightsquigarrow{\color{green}\beta}$ paths for any ${\color{green}\alpha}$ and ${\color{green}\beta}$, we should combine the former embedding with an embedding characterising the frequency on how the nodes' labels appear in the generated traces.} \ADD
...
At this stage, the computation of $\underset{\nonlogtrace\in \WCal{\pmin}{n}, P_\nonlogtrace\in\TBf{p}{n}(P)}{\max\arg} k_{\gorgembed}(P_\nonlogtrace, P_{\logtrace})$ returns the best approximated trace alignment $\nonlogtrace$ for a query trace represented as $P_{\logtrace}$. %\xout{Similarly, we can provide the TG $P\in\mathbf{P}$ providing the best approximated alignment for $P_{\logtrace}$ as $\underset{P}{\max\arg}\underset{ P_\trace\in\mathbf{P}_p^n(P)}{\max} k_{\gorgembed}(P_\trace, P_{\logtrace})$.}¯
...
\begin{table}[!t]
	\centering
	\caption{Embedding for both the traces $\nonlogtrace$ of maximum length $4$ from $P$ and  $\logtrace$.}\label{tab:emb2}\label{tab:embsitar}
...
%\xout{After defining the embedding, we can show that this embedding establishes some desired features that are independent of the definition of $\epsilon$ and $\nu$, and that $\epsilon$ and $\nu$ only depend on the characterization of both the labeling $L$ and the transition matrix $R$. We provide a rewriting proposition that is going to be used in the incoming subsection to provide the aforementioned characterizing properties.} \ADD
...
%\begin{example} \small\label{ex:moreskew}
...
%\begin{example} \small\label{ex:cmpexample}

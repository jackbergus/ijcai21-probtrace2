...
\ADD{$K$-nearest neighbours is a well-known problem in database theory \cite{Altman} which has the goal of finding the $k$ nearest data points to a data element $x$ from a set $\mathcal{X}$ of data points via a distance function $d$. Such queries can be efficiently performed by representing $\mathcal{X}$ and $x$ as vectors and stored in  in indexed n-ary trees (Vp-Trees \cite{Fu2000}, Kd-Trees \cite{Maneewongvatana99}). Usually, both the elements of the dataset $\mathcal{X}$ and $x$ are represented as vectors via an embedding function $\phi$, while the distance $d$ can be either the Euclidean distance or an ad-hoc vector distance. The main goal of such vectorial representation is that the vectors $\phi(x_i)$ for $x_i\in \mathcal{X}$ shall not be changed when a new query $x$ is performed. In the next two subsections, we're going to discuss how can we intuitively express both probabilistic trace alignment strategies as a $k$-nearest beighbours problem.}

...
The last step takes the so-obtained model traces and ranks them by considering their probabilities and the similarity with the trace $\trace$ of interest. In Figure~\ref{fig:pipe} this is shown as a black-box. As we describe in detail next, we have implemented this last step in two alternative ways: one computationally demanding but guaranteeing to produce an optimal ranking, the other more efficient but providing approximate ranking without optimality guarantees.\todo{Ho tagliato la descrizione che c'era dopo. Al limite la incorporerei nella rispettiva sottosezione.}

...
\begin{document}
	...
	\node (init1) {};
	\node[state,right=3mm of init1,label=above:\color{green}$c$] (s1) {1};
	\node[state,right=of s1,label=above:\color{green}$a$]       (s2) {2};
	\node[state,right=of s2,label=above:\color{green}$c$]       (s3) {3};
	\node[state,accepting,right=of s3,label=above:\color{green}$b$]       (s4) {3};
	
	\draw[link] (init1) -- (s1);
	\draw[link] (s1) -- node[midway,above]{\color{violet}$1$} (s2);
	\draw[link] (s2) -- node[midway,above]{\color{violet}$1$} (s3);
	\draw[link] (s3) -- node[midway,above]{\color{violet}$1$} (s4);
	\end{tikzpicture}
\end{document}

...
\node[state,accepting,right=of b1,label=above:\color{gray}$\tau$]  (f) {7};


\draw[link] (s) -- node[midway,above]{\color{violet}\pa} (bp2);
...
\draw (b2) edge [link] node[midway,above]{\color{violet}\pd} (a2);
\draw[link] (a2) -- node[midway,right]{\color{violet}\pc} (b1);
\draw[link] (a2) -- node[midway,below]{\color{violet}\pf} (f);
\draw[link] (b2) -- node[midway,above]{\color{violet}\pe} (b);
\draw[link] (b) -- node[midway,below]{\color{violet}1} (f);
